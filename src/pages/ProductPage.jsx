...
import { allProducts, categoryRouteMap, productData } from "../data/storefront-content";
import {
  compressImageFile,
  copyText,
  formatCurrency,
...
const POLICY_SECTIONS = [
  {
    ...
    getBody: (product) => `${product.dispatchText}. ${product.deliveryText}. ${product.shippingText}.`
  },
  {
    key: "returns",
    title: "Return & Refund",
  ...
  },
  {
    ...
    getBody: (product) => `${product.codText}. Availability depends on serviceability and order value for your delivery location.`
  }
];
...
export default function ProductPage({ context }) {
  ...
  useEffect(() => {
    ...
    document.title = `Avyona | ${product.name}`;
  }, [product]);
  ...
  useEffect(() => {
    if (activeMedia.type !== "image") return undefined;
  ...
  const handlePincodeCheck = (event) => {
    ...
    setDeliveryMessage(`Delivery to ${trimmed} is available in 3-7 business days. Dispatch starts within 24-48 hours.`);
  };
          ...
          <div className="avy-summary-card">
            ...
            <div className="avy-price-block">
              <div className="avy-price-line">
                ...
                <span className="avy-mrp">{formatCurrency(mrp)}</span>
            ...
            <section className="avy-payment-card">
              ...
              <div className="avy-payment-grid">
                {PAYMENT_LOGOS.map((logo) => (
                  <div key={logo.src} className="avy-payment-icon">
