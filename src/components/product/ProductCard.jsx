      ...
      <div className="product-card-footer">
        <div className="card-pricing">
          <div className="card-price-line">
            <strong>{formatCurrency(firstVariant?.price ?? product.price)}</strong>
            <span className="card-mrp">{formatCurrency(firstVariant?.mrp ?? product.mrp)}</span>
          </div>
        ...
        {actionMode === "link" ? (
          <Link className="add-to-cart" to={`/product/${product.slug}`}>{actionLabel}</Link>
        ) : (
