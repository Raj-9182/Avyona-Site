...
export function getStorefrontBaseUrl() {
  const configuredUrl = import.meta.env.VITE_STOREFRONT_URL || "";
...
export function buildStorefrontProductUrl(productOrSlug, variantOrKey) {
  return `${getStorefrontBaseUrl()}${buildProductPath(productOrSlug, variantOrKey)}`;
}
