      ...
      <section className="profile-shell">
        ...
        <section className="profile-layout">
          ...
          <div className="profile-content">
            ...
            <section className="profile-card"><div className="profile-section-head"><div><p className="eyebrow">My Orders</p><h2>Recent purchases</h2></div></div><div className="profile-list">{context.orders.length ? context.orders.map((order, index) => <article key={`${order.slug}:${index}`} className="profile-item"><ProfileProductImage src={order.image} alt={order.name} /><div className="profile-item-copy"><h3>{order.name}</h3><p>{order.category}</p><div className="profile-order-meta"><span>{order.date}</span><span>{order.status}</span><span>{`Qty: ${order.quantity}`}</span></div></div><strong className="profile-item-price">{formatCurrency(order.total)}</strong></article>) : <div className="profile-empty"><h3>No orders yet</h3><p>Your completed purchases will appear here once you place an order.</p></div>}</div></section>
            <section className="profile-card"><div className="profile-section-head"><div><p className="eyebrow">Wishlist</p><h2>Saved products</h2></div></div><div className="profile-list">{context.wishlist.length ? context.wishlist.map((item) => <article key={`${item.slug}:${item.variantLabel || ""}`} className="profile-item"><ProfileProductImage src={item.image} alt={item.name} /><div className="profile-item-copy"><h3>{item.name}</h3><p>{item.category}{item.variantLabel ? ` | ${item.variantLabel}` : ""}</p><span>Saved to wishlist</span></div><strong className="profile-item-price">{formatCurrency(item.price)}</strong></article>) : <div className="profile-empty"><h3>No wishlist items</h3><p>Your saved products will appear here when you add them from any product page.</p></div>}</div></section>
            <section className="profile-card"><div className="profile-section-head"><div><p className="eyebrow">Cart</p><h2>Added to Cart</h2></div></div><div className="profile-list">{context.cart.length ? context.cart.map((item) => <article key={`${item.slug}:${item.variantLabel || ""}`} className="profile-item"><ProfileProductImage src={item.image} alt={item.name} /><div className="profile-item-copy"><h3>{item.name}</h3><p>{item.category}{item.variantLabel ? ` | ${item.variantLabel}` : ""}</p><span>{`Qty: ${item.quantity}`}</span></div><strong className="profile-item-price">{formatCurrency(Number(item.price || 0) * Number(item.quantity || 1))}</strong></article>) : <div className="profile-empty"><h3>No cart items</h3><p>Your added products will appear here before checkout.</p></div>}</div></section>
          </div>
        </section>
      </section>
    </main>
